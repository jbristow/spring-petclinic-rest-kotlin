...
class VisitRestControllerTests {
    ...
    fun testGetVisitSuccess() {
        given<Visit>(clinicService.findVisitById(2)).willReturn(visit2)
        mockMvc.perform(
            get("/api/visits/2")
    ...
    fun testGetVisitNotFound() {
        given<Visit>(clinicService.findVisitById(-1)).willReturn(null)
        mockMvc.perform(
            get("/api/visits/-1")
    ...
    fun testGetAllVisitsSuccess() {
        given(clinicService.findAllVisits()).willReturn(listOf(visit2, visit3))
        mockMvc.perform(
            get("/api/visits/")
                .accept(MediaType.APPLICATION_JSON)
        )
    ...
    fun testGetAllVisitsNotFound() {
        given(clinicService.findAllVisits()).willReturn(emptyList())
        mockMvc.perform(
            get("/api/visits/")
                .accept(MediaType.APPLICATION_JSON)
        )
