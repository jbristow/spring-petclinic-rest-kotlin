...
class SpecialtyRestControllerTests {
    ...
    fun testGetSpecialtySuccess() {
        given<Specialty>(clinicService.findSpecialtyById(1)).willReturn(radiology)
        mockMvc.perform(
    ...
    fun testGetSpecialtyNotFound() {
        given<Specialty>(clinicService.findSpecialtyById(-1)).willReturn(null)
        mockMvc.perform(
            get("/api/specialties/-1")
    ...
    fun testGetAllSpecialtysSuccess() {
        given(clinicService.findAllSpecialties()).willReturn(listOf(surgery, dentistry))
        mockMvc.perform(
            get("/api/specialties/")
                .accept(MediaType.APPLICATION_JSON)
        )
    ...
    fun testUpdateSpecialtySuccess() {
        given<Specialty>(clinicService.findSpecialtyById(2))
            .willReturn(surgery)
