...
class VetRestControllerTests {
    ...
    fun testGetVetSuccess() {
        given<Vet>(clinicService.findVetById(1)).willReturn(Vet(vet1))
        mockMvc.perform(
            get("/api/vets/1")
    ...
    fun testGetVetNotFound() {
        given<Vet>(clinicService.findVetById(-1)).willReturn(null)
        mockMvc.perform(
            get("/api/vets/-1")
    ...
    fun testGetAllVetsSuccess() {
        given(clinicService.findAllVets()).willReturn(listOf(vet1, vet2))
        mockMvc.perform(
            get("/api/vets/")
                .accept(MediaType.APPLICATION_JSON)
        )
    ...
    fun testGetAllVetsNotFound() {
        given(clinicService.findAllVets()).willReturn(emptyList())
        mockMvc.perform(
            get("/api/vets/")
                .accept(MediaType.APPLICATION_JSON)
        )
